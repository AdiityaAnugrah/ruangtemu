...
export default function AdminCitiesPage() {
  ...
  const { data: requests = [] } = useQuery({
    queryKey: ["city-requests"],
    queryFn: () => citiesApi.listRequests().then((r) => r.data),
    enabled: isAdmin(),
  });
  ...
  const startCreate = (cityName = "") => {
    setEditingId(null);
    setForm({ name: cityName, areasText: "", isActive: true });
    setShowCreate(true);
  };
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="mx-auto max-w-6xl px-4 py-8">
        <div className="mb-5 flex flex-col gap-3 rounded-lg border border-slate-200 bg-white p-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 className="text-base font-bold text-slate-950">Kota dan Area Layanan</h2>
            <p className="mt-1 text-sm text-slate-500">Tambah kota baru, edit area ready, atau nonaktifkan lokasi.</p>
          </div>
          <button
            onClick={() => startCreate()}
        ...
        {isLoading ? (
          ...
          <div className="grid gap-4 lg:grid-cols-[1fr_320px]">
            ...
            <aside className="rounded-lg border border-slate-200 bg-white p-4">
              <h3 className="text-sm font-bold text-slate-950">Permintaan Kota</h3>
              <p className="mt-1 text-xs text-slate-500">Kota yang diminta user saat wilayah belum tersedia.</p>
              <div className="mt-4 space-y-2">
                {requests.length === 0 ? (
                  <p className="rounded-lg bg-slate-50 px-3 py-4 text-center text-xs text-slate-500">Belum ada request.</p>
                ) : requests.slice(0, 8).map((request: any) => (
                  <button
                    key={request.id}
                    onClick={() => startCreate(request.cityName)}
                    className="flex min-h-11 w-full items-center justify-between rounded-lg border border-slate-200 px-3 text-left text-sm hover:bg-slate-50"
                  >
                    <span className="font-medium text-slate-700">{request.cityName}</span>
                    <span className="rounded-full bg-slate-100 px-2 py-0.5 text-xs font-semibold text-slate-500">{request.count}x</span>
                  </button>
                ))}
              </div>
            </aside>
